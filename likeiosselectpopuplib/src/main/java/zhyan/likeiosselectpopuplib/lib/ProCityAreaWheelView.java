...
import zhyan.likeiosselectpopuplib.adapter.WheelAdapter;
import zhyan.likeiosselectpopuplib.listener.OnItemSelectedListener;
import zhyan.likeiosselectpopuplib.model.IPickerViewData;

/**
 * 3d滚轮控件
 */
public class ProCityAreaWheelView extends View {
    ...
    public ProCityAreaWheelView(Context context) {
        this(context, null);
    }

    public ProCityAreaWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initLoopView(Context context) {
        ...
        handler = new ProCityAreaMessageHandler(this);
        gestureDetector = new GestureDetector(context, new ProCityAreaLoopViewGestureListener(this));
    ...
    void smoothScroll(ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new ProCityAreaSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    protected final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new ProvCityAreaInertiaTimerTask(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
    ...
    protected final void onItemSelected() {
        if (onItemSelectedListener != null) {
            postDelayed(new ProvCityAreaOnItemSelectedRunnable(this), 200L);
        }
    }
