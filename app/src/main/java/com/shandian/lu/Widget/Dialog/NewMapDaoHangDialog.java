...
public class NewMapDaoHangDialog extends Dialog {
    Context context;

    public interface DialogCallBackListener{//通过该接口回调Dialog需要传递的值
        public void callBack(boolean isBaidu);//具体方法
    }
    public NewMapDaoHangDialog(Context context1) {
        super(context1);
        this.context = context1;


    }
    public NewMapDaoHangDialog(Context context1, int themeResId) {
        super(context1, themeResId);
        this.context = context1;
    }

    protected NewMapDaoHangDialog(Context context1, boolean cancelable, OnCancelListener cancelListener) {
    ...
    public  class Builder {
         ...
         * 4，显示dialog的布局
         */
        public NewMapDaoHangDialog build(Context context) {
            LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final NewMapDaoHangDialog companyCustomTelDialog = new NewMapDaoHangDialog(context, R.style.MyDialogStyle);//默认调用带style的构造
            companyCustomTelDialog.setCanceledOnTouchOutside(true);//默认点击布局外不能取消dialog
            View view = mInflater.inflate(R.layout.dialog_map_daohang, null);
            ...
            LinearLayout llyBaidu = (LinearLayout) view.findViewById(R.id.lly_dialog_map_daohang_baidu);
            LinearLayout llyGaoDe = (LinearLayout) view.findViewById(R.id.lly_dialog_map_daohang_gaode);
            llyBaidu.setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            llyGaoDe.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if(mDialogCallBackListener != null){
