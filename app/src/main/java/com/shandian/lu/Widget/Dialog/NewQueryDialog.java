...
public class NewQueryDialog extends Dialog {
    Context context;
    private String content;
    public interface DialogCallBackListener{//通过该接口回调Dialog需要传递的值
        public void callBack(boolean isQuery);//具体方法
    }
    public NewQueryDialog(Context context1,String content1) {
        super(context1);
        this.context = context1;
        content = content1;

    }
    public NewQueryDialog(Context context1, int themeResId) {
        super(context1, themeResId);
        this.context = context1;
    }

    protected NewQueryDialog(Context context1, boolean cancelable, OnCancelListener cancelListener) {
    ...
    public  class Builder {
         ...
         * 4，显示dialog的布局
         */
        public NewQueryDialog build(Context context) {
            LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final NewQueryDialog companyCustomTelDialog = new NewQueryDialog(context, R.style.MyDialogStyle);//默认调用带style的构造
            companyCustomTelDialog.setCanceledOnTouchOutside(true);//默认点击布局外不能取消dialog
            View view = mInflater.inflate(R.layout.dialog_new_query, null);
            ...
            TextView tvContent = (TextView) view.findViewById(R.id.tv_new_query_content);
            tvContent.setText("确认"+content);
