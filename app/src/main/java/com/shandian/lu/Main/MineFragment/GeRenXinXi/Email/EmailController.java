...
public class EmailController extends BaseController {
    ...
    @BindView(R.id.rly_main_mine_gerenxinxi_email_topbar_back)
    RelativeLayout rlyMainMineGeRenXinXiEmailTopBarBack;
    @OnClick(R.id.rly_main_mine_gerenxinxi_email_topbar_back)
    public void  rlyMainMineGeRenXinXiEmailTopBarBackOnclick(){
        activity.finish();
    }
    @BindView(R.id.pb_new_main_mine_gerenxinxi_email)
    ProgressBar pbNewMainMineGeRenXinXiEmail;
    @BindView(R.id.et_main_mine_gerenxinxi_email)
    EditText etMainMineGeRenXinXiEmail;
    @BindView(R.id.bt_main_mine_gerenxinxi_email_submit)
    Button btMainMineGeRenXinXiEmailSubmit;
    @OnClick(R.id.bt_main_mine_gerenxinxi_email_submit)
    public void btMainMineGeRenXinXiEmailSubmitOnclick(){
        /*updateEmailToNet();*/
        newUpdateToNet();
    }
    public EmailController(Activity activity1){
        activity = activity1;
        init();
    }
    ...
    private Map<String,String> getParamMap(){
        ...
        paramMap.put("image","");
        String email = etMainMineGeRenXinXiEmail.getText().toString().trim();
        if(email == null){
            email = "";
        }else{
    ...
    private void newUpdateToNet(){
        pbNewMainMineGeRenXinXiEmail.setVisibility(View.VISIBLE);
        UserNetWork userNetWork = new UserNetWork();
        userNetWork.submitNewGeRenXinXiToNet(getNewParamMap(), new Observer<NewGeRenXinXiSubmitBean>() {
            ...
            public void onNext(NewGeRenXinXiSubmitBean newGeRenXinXiSubmitBean) {
                ...
                pbNewMainMineGeRenXinXiEmail.setVisibility(View.GONE);
            }
        });
