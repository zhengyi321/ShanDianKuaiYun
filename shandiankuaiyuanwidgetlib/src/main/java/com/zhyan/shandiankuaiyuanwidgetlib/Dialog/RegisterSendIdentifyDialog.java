...
public class RegisterSendIdentifyDialog extends Dialog {
    Context context;
    String tel;
    public interface DialogCallBackListener{//通过该接口回调Dialog需要传递的值
        public void callBack(String tel);//具体方法
    }
    public RegisterSendIdentifyDialog(Context context1,String tel) {
        super(context1);
        this.context = context1;
        this.tel = tel;
    }
    public RegisterSendIdentifyDialog(Context context1, int themeResId) {
        super(context1, themeResId);
        this.context = context1;
    }

    protected RegisterSendIdentifyDialog(Context context1, boolean cancelable, OnCancelListener cancelListener) {
    ...
    public  class Builder {
         ...
         * 4，显示dialog的布局
         */
        public RegisterSendIdentifyDialog build(Context context) {
            LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final RegisterSendIdentifyDialog companyCustomTelDialog = new RegisterSendIdentifyDialog(context, R.style.MyDialogStyle);//默认调用带style的构造
            companyCustomTelDialog.setCanceledOnTouchOutside(true);//默认点击布局外不能取消dialog
            View view = mInflater.inflate(R.layout.dialog_register_sms_send_rly, null);
            ...
            final TextView tvTel= (TextView) view.findViewById(R.id.tv_register_sms_send_tel);
            if(tel != null){
                tvTel.setText(tel);
            }
            if (!TextUtils.isEmpty(positiveText)) {//这是确认按钮
                RelativeLayout rly_cofirm = (RelativeLayout) view.findViewById(R.id.rly_register_sms_send_query);
                ...
                view.findViewById(R.id.rly_register_sms_send_query).setVisibility(View.GONE);
            }
            if (!TextUtils.isEmpty(negativeText)) {//这是取消按钮逻辑处理
                RelativeLayout btn_cancle = (RelativeLayout) view.findViewById(R.id.rly_register_sms_send_cancel);
                ...
                view.findViewById(R.id.rly_register_sms_send_cancel).setVisibility(View.GONE);
            }
