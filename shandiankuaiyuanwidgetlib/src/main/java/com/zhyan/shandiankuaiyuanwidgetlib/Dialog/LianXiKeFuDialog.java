...
public class LianXiKeFuDialog extends Dialog {
    Context context;

    public interface DialogCallBackListener{//通过该接口回调Dialog需要传递的值
        public void callBack(String tel);//具体方法
    }
    public LianXiKeFuDialog(Context context1) {
        super(context1);
        this.context = context1;

    }
    public LianXiKeFuDialog(Context context1, int themeResId) {
        super(context1, themeResId);
        this.context = context1;
    }

    protected LianXiKeFuDialog(Context context1, boolean cancelable, OnCancelListener cancelListener) {
    ...
    public  class Builder {
         ...
         * 4，显示dialog的布局
         */
        public LianXiKeFuDialog build(Context context) {
            LayoutInflater mInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            final LianXiKeFuDialog companyCustomTelDialog = new LianXiKeFuDialog(context, R.style.MyDialogStyle);//默认调用带style的构造
            companyCustomTelDialog.setCanceledOnTouchOutside(true);//默认点击布局外不能取消dialog
            View view = mInflater.inflate(R.layout.dialog_lianxikefu, null);
            ...
            final TextView tvTel= (TextView) view.findViewById(R.id.tv_lianxikefu_tel);

            if (!TextUtils.isEmpty(positiveText)) {//这是确认按钮
                RelativeLayout rly_cofirm = (RelativeLayout) view.findViewById(R.id.rly_lianxikefu_query);
            ...
            if (!TextUtils.isEmpty(negativeText)) {//这是取消按钮逻辑处理
                RelativeLayout btn_cancle = (RelativeLayout) view.findViewById(R.id.rly_lianxikefu_cancel);
